...
vim.opt.tabstop = 4 -- Or your preferred number of spaces
vim.opt.shiftwidth = 4 -- Or your preferred number of spaces
...
vim.api.nvim_create_autocmd("FileType", {
	pattern = "markdown",
	desc = "Configure markdown editing behavior",
	callback = function()
		-- Disable hard-wrapping (no automatic line breaks)
		vim.bo.textwidth = 0
		...
		-- Soft wrap for visual display (doesn't change file)
		vim.wo.wrap = true
		vim.wo.linebreak = true  -- Break at word boundaries

		-- Hide markdown syntax for cleaner view
		vim.wo.conceallevel = 2

		-- Auto-format: continue lists and blockquotes
		vim.bo.formatoptions = vim.bo.formatoptions .. "ro"
		vim.bo.comments = "b:*,b:-,b:+,b:>,fb:-"

		-- Folding by headers (use za to toggle, zM to close all, zR to open all)
		vim.wo.foldmethod = "expr"
		vim.wo.foldexpr = "v:lua.vim.treesitter.foldexpr()"
		vim.wo.foldenable = false  -- Start with folds open

		-- Navigate wrapped lines like real lines
		...
		vim.keymap.set("n", "gx", function()
			...
			-- If not found, try to extract bare URL under cursor
			if not url then
				-- Find URL pattern around cursor position
				local patterns = {
					"https?://[%w-_%.%?%.:/%+=&]+",
					"www%.[%w-_%.%?%.:/%+=&]+",
				}
		...
		end, { buffer = true, desc = "Open link under cursor" })
	end,
})
...
require("lazy").setup({
  ...
  keys = {
	...
	-- Obsidian vault integration
	{
		...
		lazy = true,
		event = {
			"BufReadPre " .. vim.fn.expand("~") .. "/Documents/aspengrove/**.md",
			"BufNewFile " .. vim.fn.expand("~") .. "/Documents/aspengrove/**.md",
		},
		dependencies = {
			"nvim-lua/plenary.nvim",
		},
		opts = {
			...
			-- Disable frontmatter management if you don't use it
		...
		keys = {
			...
			{ "<leader>oi", "<cmd>ObsidianTemplate<cr>", desc = "[O]bsidian [I]nsert template" },
		},
	},
	...
	-- Markdown preview in browser
	{
		...
		config = function()
			...
			vim.g.mkdp_refresh_slow = 0 -- Refresh on typing, not just on save
	...
	-- Table mode for easy table creation
	{
		...
		ft = { "markdown" },
		config = function()
			vim.g.table_mode_corner = "|" -- Use standard markdown table format

			-- Keymap to toggle table mode
	...
	-- Enhanced markdown syntax
	{
		...
		ft = { "markdown" },
		config = function()
			vim.g.vim_markdown_folding_disabled = 1 -- Use treesitter folding instead
			...
			vim.g.vim_markdown_strikethrough = 1 -- Support ~~strikethrough~~
			vim.g.vim_markdown_new_list_item_indent = 2 -- Indent for new list items
			vim.g.vim_markdown_auto_insert_bullets = 1 -- Auto-insert bullets
			vim.g.vim_markdown_follow_anchor = 1 -- Follow anchors with ge
		end,
	},
	...
	-- Table of contents generation
	{
		...
		ft = { "markdown" },
		config = function()
			vim.g.vmt_auto_update_on_save = 1 -- Auto-update TOC on save
			vim.g.vmt_fence_text = "TOC" -- Fence text for TOC
			vim.g.vmt_fence_closing_text = "/TOC" -- Closing fence text
		end,
	},
	...
	-- Image pasting from clipboard
	{
		...
		keys = {
			{ "<leader>p", "<cmd>PasteImage<cr>", desc = "[P]aste image from clipboard", mode = { "n", "i" } },
		},
	},
